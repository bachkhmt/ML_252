...
* **Kích thước:** 129,880 mẫu × 23 đặc trưng (22 Features + 1 Target), chia sẵn thành train (103,904 mẫu) và test (25,976 mẫu)

### 📊 Đặc điểm dữ liệu
- **✓ Có missing values**: Cột `Arrival Delay in Minutes` có 310 giá trị bị thiếu (~0.3%), là cơ hội thực hành **Imputation** (điền giá trị thiếu bằng Mean, Median hoặc KNN Imputer).
- **✓ Có categorical features**: Gồm 4 đặc trưng dạng chữ (`Gender`, `Customer Type`, `Type of Travel`, `Class`) phục vụ thực hành **Encoding** (One-Hot hoặc Label Encoding).
- **✓ Có ordinal/rating features**: 14 đặc trưng dạng thang điểm 0–5 (như `Inflight wifi service`, `Seat comfort`, `Cleanliness`...) phản ánh mức độ hài lòng từng dịch vụ.
- **✓ Kích thước mẫu đủ lớn**: Tổng cộng **129,880** mẫu, đảm bảo Pipeline có ý nghĩa thống kê cao.
- **✓ Bài toán**: **Binary Classification** (Phân loại nhị phân: Dự đoán hành khách `satisfied` hay `neutral or dissatisfied`).
